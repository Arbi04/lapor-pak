...
class ResidentController extends Controller
{
    ...
    public function update(UpdateResidentRequest $request, string $id)
    {
        ...
        $oldResident = $this->residentRepository->getResidentById($id);

        if ($request->hasFile('avatar')) {
            if ($oldResident->avatar && Storage::disk('public')->exists($oldResident->avatar)) {
                Storage::disk('public')->delete($oldResident->avatar);
            }
